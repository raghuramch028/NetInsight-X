...
* **Environment-driven configuration:** `DJANGO_SECRET_KEY`, `DEBUG`, `ALLOWED_HOSTS`, `NETINSIGHT_DB_PATH`, `NETINSIGHT_SVM_PATH`, `NETINSIGHT_DEMO_MODE`, and `NETINSIGHT_LOG_LEVEL` are read from the environment at startup, making the project easily configurable without editing source files.
