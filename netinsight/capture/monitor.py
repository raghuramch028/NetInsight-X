...
DEFAULT_METRICS_WINDOW_SECONDS = 2.0
DEFAULT_DEMO_CYCLE_SECONDS = 30.0
...
class LiveMonitor:
    ...
    def __init__(self, queue_maxsize: int = DEFAULT_QUEUE_MAXSIZE):
        self.packet_queue = queue.Queue(maxsize=queue_maxsize)
    ...
    def get_capture_rate(self) -> float:
        ...
        with self._counter_lock:
            captured = self.total_captured_packets
            dropped = self.total_dropped_packets
        processed = captured + dropped
        if processed == 0:
            return 100.0
        return (captured / processed) * 100.0
